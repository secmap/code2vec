...
def _compute_sampled_logits(weights,
  ...
  with ops.name_scope(name, "compute_sampled_logits",
    ...
    print('rank_matrix.shape = ', rank_matrix.shape)
    # all_ids_indice is a [batch_size * num_true + num_sampled, num_hash_func] int tensor
    all_ids_indice = embedding_ops.embedding_lookup(
        rank_matrix, all_ids, partition_strategy=partition_strategy)

    print('all_ids_indice.shape = ', all_ids_indice.shape)

    # weights shape is [len(all_ids), hash_func_max, embedding_size]
    all_w = embedding_ops.embedding_lookup(
        weights, all_ids_indice, partition_strategy=partition_strategy)
    all_b = embedding_ops.embedding_lookup(
        biases, all_ids_indice, partition_strategy=partition_strategy)
    ...
    print('all_b.shape(after reshape) = ', all_b.shape)

    # true_w shape is [batch_size * num_true, dim]
    # true_b is a [batch_size * num_true] tensor
    true_w = array_ops.slice(
        all_w, [0, 0], array_ops.stack([array_ops.shape(labels_flat)[0] * num_hash_func, -1]))
    true_b = array_ops.slice(all_b, [0], array_ops.shape(labels_flat) * num_hash_func)

    print('true_w.shape = ', true_w.shape)
    print('true_b.shape = ', true_b.shape)

    print('inputs.shape = ', inputs.shape)
    inputs_tile = array_ops.tile(inputs, [1, num_hash_func])
    inputs_tile = array_ops.reshape(inputs_tile, [-1, embedding_size])
    print('inputs_tile.shape = ', inputs_tile.shape)
    ...
    print('row_wise_dots.shape = ', row_wise_dots.shape)
    ...
    print('true_logits.shape = ', true_logits.shape)
    ...
    print('sampled_logits.shape = ', sampled_logits.shape)
    ...
    if subtract_log_q:
      print('true_expected_count.shape = ', true_expected_count.shape)
      true_expected_count_tile = array_ops.tile(true_expected_count, [1, num_hash_func])
      true_expected_count_tile = array_ops.reshape(true_expected_count_tile, [-1, 1])
      print('sampled_expected_count.shape = ', sampled_expected_count.shape)
      sampled_expected_count_tile = array_ops.reshape(sampled_expected_count, [-1, 1])
      sampled_expected_count_tile = array_ops.tile(sampled_expected_count_tile, [1, num_hash_func])
      sampled_expected_count_tile = array_ops.reshape(sampled_expected_count_tile, [-1])
      # Subtract log of Q(l), prior probability that l appears in sampled.
      true_logits -= math_ops.log(true_expected_count_tile)
      sampled_logits -= math_ops.log(sampled_expected_count_tile)

    true_logits = array_ops.reshape(true_logits, [-1, num_hash_func])
